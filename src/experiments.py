...
class ArticleDeconvolutionExample(Experiment):
    ...
    def make_models(self) -> Experiment:
        ...
        commom_params = {
            ...
            "learning_rate": lambda i: np.float64(0.5/np.sqrt(i)),
...
class NAGDeconvolutionExample(Experiment):
    """This experiment is aimed at recreating the the article's plots for the
    deconvolution example."""
    path = PROJECT_ROOT / "runs" / "nag_deconvolution_example"
    path.mkdir(parents=True, exist_ok=True)
    ...
    def make_model(self) -> Experiment:
        ...
        self.model = NAGSIPDeconvolution1D(**params)
        self.has_model = True
        self.params = self.get_experiment_params_dict()

    def save_model(self):
        self.logger.info("Saving fitted models.")
        with open(f"{self.model.name}_model.pkl", "wb") as file:
            pickle.dump(self.model, file)
    ...
    def fit_model(
        ...
        """Fit models."""
        assert self.has_model
        self.logger.info(f"Fitting {self.model.name} model.")
    ...
    def make_target_plot(self, plots_path: Path):
        ...
        ax.plot(
            plot_x,
            self.model.predict(plot_x),
            "-",
            label=f"{self.model.name}'s estimate"
        )
        ax.set_xlabel("w")
        ax.legend()
        fig.savefig(plots_path / "truth_and_estimates.pdf")
